...
describe('update command subprocess with mock registry', () => {
  ...
  const handleUpdateMockRegistryRequest = (
    url: string,
  ...
  beforeAll(async () => {
    const server = createServer((request, response) => {
      ...
      if (handleUpdateMockRegistryRequest(url, response)) {
        return;
      }
...
describe('update command multi-target verbose summary', () => {
  ...
  it('prints dry-run summary with --verbose and multiple targets', async () => {
    const cwd = mkdtempSync(path.join(os.tmpdir(), 'agents-update-cli-multitarget-dry-run-verbose-'));
    tempDirs.push(cwd);
    ...
    const result = await runCliSubprocess(['--verbose', '--dry-run', 'update'], { cwd });
